...
import { CalendarIcon, Eye, EyeOff, Loader2, Save, Upload } from 'lucide-react';
...
import type { UpdateEmployeePayload } from '@/lib/api/employees';
...
type EditEmployeeFormProps = {
    employee?: EditEmployeeFormData | null;
    positions?: PositionOption[];
    branches?: BranchOption[];
    onCancel?: () => void;
    onSubmit?: (data: UpdateEmployeePayload) => void | Promise<void>;
};
...
export function EditEmployeeForm({
    ...
    useEffect(() => {
        if (employee) {
            ...
            setPositionId(employee.position_id != null ? String(employee.position_id) : '');
            setBranchId(employee.branch_id != null ? String(employee.branch_id) : '');
            setDateOfBirth(employee.date_of_birth ? new Date(employee.date_of_birth) : undefined);
            setDateHired(employee.date_hired ? new Date(employee.date_hired) : undefined);
            setBasicSalary(employee.basic_salary ?? '');
            setRfid(employee.rfid ?? '');
            setIsActive((employee.status ?? 'active') === 'active');
            setSss(employee.sss ?? '');
    ...
    const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!file || !ALLOWED_PHOTO_TYPES.includes(file.type) || file.size > MAX_PHOTO_SIZE) return;
                            ...
                            <div className="grid gap-4 sm:grid-cols-3">
                                <div className="space-y-2">
                                    ...
                                    <Input
                                        ...
                                        className={cn(inputClassName, 'bg-neutral-100 dark:bg-neutral-900')}
                ...
                <CardContent className="space-y-4">
                    <div className="grid gap-4 sm:grid-cols-2">
                        <div className="space-y-2">
                            ...
                            <Select defaultValue={employee?.role ?? 'employee'}>
                                <SelectTrigger
                                    className={cn(
                                        inputClassName,
                                        'ring-offset-2 focus-visible:border-text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring'
